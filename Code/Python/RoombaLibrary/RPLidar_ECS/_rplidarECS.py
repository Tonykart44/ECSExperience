...
### additions ECS - 08/04/2016 ###
    
    def scan_data(self):
        """Returns a list of the most recently scanned data, in format data_n = [degrees_n, mm_n]"""
        
        dis = self.current_frame.distance
        deg = self.current_frame.angle_d
        
        data = []
        
        for i in range(0,len(dis)):
            data.append([deg[i], dis[i]])
    ...
    def get_scan(self):
#        rplidar = RPLidar("COM4")    
#        rplidar.connect()

        self.start_monitor(archive=False)
        if self.raw_frames.qsize() >= 1:
            self.stop_monitor()
        
        data = self.scan_data()
        return data
        
    def data_filter(self,data,lim_high,lim_low):
        """Returns range of data within a certain viewing angle, with rounded angles"""
        ...
        for i in range(0, len(data)):
            ...
            if deg_i <= lim_high and deg_i >= lim_low:
                filtered_data.append([deg_i, dis_i])
        
        return filtered_data
        
    def get_curve(self,data,lim_high,lim_low):
        """Returns a tangent line for a curved feature, within a certain viewing angle"""
        
        filtered_data = self.data_filter(data,lim_high,lim_low)
        ...
        for i in range(0, len(filtered_data)):
            deg_i = filtered_data[i][0]
            if deg_i == lim_high:
                p1 = filtered_data[i]
            middle = lim_high - lim_low            
            if deg_i >= middle-1 and deg_i <= middle+1:
                p2 = filtered_data[i]
            if deg_i == lim_low:
                p3 = filtered_data[i]
        ...
        z = np.polyfit(x, y, 3)
        f = np.poly1d(z)
        # calculate new x's and y's, x[-1] returns the last points of the list
...
#
#
#    rplidar.start_monitor(archive=False)
#    
